...
void RouteToNode::setBloodVesselVolume(ImagePtr bloodVesselVolume)
{
	mBloodVesselVolume = bloodVesselVolume;
}

Eigen::MatrixXd RouteToNode::getCenterlinePositions(vtkPolyDataPtr centerline_r)
{
...
void RouteToNode::setSmoothing(bool smoothing)
{
	mSmoothing = smoothing; // default true
}

void RouteToNode::processCenterline(MeshPtr mesh)
{
	if (mBranchListPtr)
...
void RouteToNode::setBranchList(BranchListPtr branchList)
{
	mBranchListPtr = branchList;
}

void RouteToNode::processBloodVesselCenterline(Eigen::MatrixXd positions)
{
...
void RouteToNode::findClosestPointInBranches(Vector3D nodeCoordinate_r)
{
	...
	for (int i = 0; i < branches.size(); i++)
	{
		...
		{
                        double D = findDistance(positions.col(j), nodeCoordinate_r);
			if (D < minDistance)
			{
				minDistance = D;
...
void RouteToNode::findClosestPointInBloodVesselBranches(Vector3D nodeCoordinate_r)
{
	double minDistance = 100000;
	int minDistancePositionIndex;
	BranchPtr minDistanceBranch;
...
void RouteToNode::findRoutePositions()
{
...
void RouteToNode::findRoutePositionsInBloodVessels()
{
...
/*
    RouteToNode::searchBranchUp is finding all positions from a given index on a branch and up
    the airway tree to the top of trachea. All positions are added to mRoutePositions, which stores
    all positions along the route-to-node.
    Before the positions are added they are smoothed by RouteToNode::smoothBranch.
*/
void RouteToNode::searchBranchUp(BranchPtr searchBranchPtr, int startIndex)
{
	if (!searchBranchPtr)
		return;
...
void RouteToNode::searchBloodVesselBranchUp(BranchPtr searchBranchPtr, int startIndex)
{
	if (!searchBranchPtr)
		return;
...
vtkPolyDataPtr RouteToNode::findRouteToNode(PointMetricPtr nodePoint)
{
        mNodePosition = nodePoint->getCoordinate();

        findClosestPointInBranches(mNodePosition);
	findRoutePositions();
...
vtkPolyDataPtr RouteToNode::findExtendedRoute(PointMetricPtr nodePoint)
{
        mNodePosition = nodePoint->getCoordinate();
	...
	{
                double extensionDistance = findDistance(mRoutePositions.front(),mNodePosition);
                Eigen::Vector3d extensionVectorNormalized = ( mNodePosition - mRoutePositions.front() ) / extensionDistance;
...
vtkPolyDataPtr RouteToNode::findRouteToNodeAlongBloodVesselCenterlines(MeshPtr bloodVesselCenterlineMesh, PointMetricPtr nodePoint)
{
	...
	Eigen::MatrixXd BVCLpoints_r = getCenterlinePositions(BVcenterline_r);
        mConnectedPointsInBVCL = findClosestBloodVesselSegments(BVCLpoints_r , mCLpoints, nodePoint->getCoordinate());
	...
	processBloodVesselCenterline(mConnectedPointsInBVCL);
        findClosestPointInBloodVesselBranches(mNodePosition);
...
vtkPolyDataPtr RouteToNode::generateAirwaysFromBloodVesselCenterlines()
{
...
bool RouteToNode::makeConnectedAirwayAndBloodVesselRoute()
{
	...
	if ( mBloodVesselRoutePositions.empty() )
	{
        ...
        if ( findDistance(mRoutePositions.front(),mNodePosition) < findDistance(mBloodVesselRoutePositions.front(),mNodePosition) )
	{
                CX_LOG_INFO() << "No improved route to node found along blood vessels";
                mMergedAirwayAndBloodVesselRoutePositions = mRoutePositions; // do not add blood vessel route if that is not leading closer to node than airway route alone
		return false;
	}
	...
	mergedPositions.insert( mergedPositions.end(), mRoutePositions.begin(), mRoutePositions.end() );
        ...
        //make extension from blood vessel to node
	double extensionPointIncrement = 0.25; //mm
        double extensionDistance = findDistance(mergedPositions.front(),mNodePosition);
        Eigen::Vector3d extensionVectorNormalized = ( mNodePosition - mergedPositions.front() ) / extensionDistance;
	int numberOfextensionPoints = int(extensionDistance / extensionPointIncrement);
	Eigen::Vector3d extensionPointIncrementVector = extensionVectorNormalized * extensionDistance / numberOfextensionPoints;
...
vtkPolyDataPtr RouteToNode::getConnectedAirwayAndBloodVesselRoute()
{
	return addVTKPoints(mMergedAirwayAndBloodVesselRoutePositions);
}


bool RouteToNode::checkIfRouteToNodeEndsAtEndOfLastBranch() // remove if not in use?
{
...
vtkPolyDataPtr RouteToNode::addVTKPoints(std::vector<Eigen::Vector3d> positions)
{
...
void RouteToNode::addRouteInformationToFile(VisServicesPtr services)
{
        QString RTTpath = services->patient()->getActivePatientFolder() + "/RouteToNodeInformation/";
	QDir RTTDirectory(RTTpath);
        if (!RTTDirectory.exists()) // Creating RouteToNodeInformation folder if it does not exist
		RTTDirectory.mkpath(RTTpath);

	QString format = timestampSecondsFormat();
        QString filePath = RTTpath + QDateTime::currentDateTime().toString(format) + "_RouteToNodeInformation.txt";
	...
	if (outfile.open(QIODevice::ReadWrite))
	{
		QTextStream stream(&outfile);
                ...
                stream << "#Node position:" << endl;
                stream << mNodePosition(0) << " " << mNodePosition(1) << " " << mNodePosition(2) << " " << endl;
		if (mProjectedBranchPtr)
		{
                        stream << "#Route to node generations:" << endl;
		...
		stream << tracheaLength << endl;
                ...
                stream << "#Route to node length - from Carina (mm):" << endl;
		stream << calculateRouteLength(mRoutePositions) - tracheaLength << endl;
                stream << "#Extended route to node length - from Carina (mm):" << endl;
		stream << calculateRouteLength(mExtendedRoutePositions) - tracheaLength << endl;
	}
}

double RouteToNode::getTracheaLength()
{
...
std::vector< Eigen::Vector3d > RouteToNode::getRoutePositions(MeshPtr route)
{
...
double RouteToNode::calculateRouteLength(std::vector< Eigen::Vector3d > route)
{
...
void RouteToNode::setBloodVesselRadius()
{
	std::vector<BranchPtr> branches = mBloodVesselBranchListPtr->getBranches();
...
double RouteToNode::calculateBloodVesselRadius(Eigen::Vector3d position, Eigen::Vector3d orientation)
{
	double radius = 0;
...
double RouteToNode::findDistanceToSegmentationEdge(vtkImageDataPtr bloodVesselImage, Eigen::Vector3i indexVector, Eigen::Vector3d perpendicularVector, int* dim, double* spacing, int direction)
{
	double retval;
...
std::vector< Eigen::Vector3d > RouteToNode::getRoutePositions()
{
...
std::vector< double > RouteToNode::getCameraRotation()
{
...
void RouteToNode::makeMarianaCenterlineFile(QString filename)
{
	...
	ofstream out(filename.toStdString().c_str());
        out << "# [xPos yPos zPos BranchingPoint (0=Normal, 1=Branching position, 2=Extended from airway to node)] ";
...
QJsonArray RouteToNode::makeMarianaCenterlineJSON()
{
	QJsonArray array;
...
/*
                RouteToNode::getBranchPositions is used to get positions of a branch without smoothing.
...
Eigen::MatrixXd findClosestBloodVesselSegments(Eigen::MatrixXd bloodVesselPositions , Eigen::MatrixXd airwayPositions, Vector3D nodePosition)
{
	...
	{
                Eigen::MatrixXd::Index closestBloodVesselPositionToNode = dsearch(nodePosition, bloodVesselPositions).first;
                std::pair< Eigen::MatrixXd, Eigen::MatrixXd > localPositions = findLocalPointsInCT(closestBloodVesselPositionToNode , bloodVesselPositions);
